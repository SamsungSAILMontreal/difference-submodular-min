...
% PADCAT - concatenate vectors with different lengths by padding with last value
...
SZ = cat(1,SZ{:}) ;
maxSZ = max(SZ, [], 1);    % probable size of the longest vector
% maxSZ equals :
...
if nargin == 1
    ...
    if all(maxSZ==1) % treat scalars as columns
        dim = 1;
    else
    ...
    elseif all(SZ(:,dim)==SZ(1,dim))
        ...
        for elt = [rows'; cols']
            if SZ(elt(2)) ~= 0
                M(elt(1), elt(2)) = M(SZ(elt(2)), elt(2)); % fill in with last non NaN value
            end
        end
    end
